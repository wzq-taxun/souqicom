  ...
  <div>
    ...
    >
      <el-row :gutter="20">
        ...
        <el-col :span="4">
          <el-button type="primary" class="buttshi" @click="goAddpage">添加新闻</el-button>
        </el-col>
      </el-row>
      ...
      <el-table :data="goodslist" border stripe>
        ...
        <el-table-column label="文章来源" prop="news_source" width="150px"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    removeById(idzhi) {
      this.$confirm('是否确定删除', '提示', {
        ...
        .then(async () => {
          ...
          const { data: res } = await this.$http.post('souqi/admin/news_manage/delete/', {
            admin_pk: detatepk,
            pk: idzhi
          })
    ...
    // 去添加新闻内容页面
    goAddpage() {
      this.$router.push({ path: '/newlist/addnew' })
    },
    async getGoodsList() {
      this.openloding()
      const { data: res } = await this.$http.post('souqi/admin/news_manage/')
      // console.log(res)
      ...
      // 将页码赋值
      if (window.sessionStorage.getItem('newPage2')) {
        this.queryInfo.pagenum = parseInt(window.sessionStorage.getItem('newPage2'))
    ...
    handleCurrentChange(newPage) {
      ...
      window.sessionStorage.setItem('newPage2', newPage)
