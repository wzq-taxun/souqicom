  ...
  methods: {
    ...
    async updatedite() {
      if (this.inputtitle.trim() === '' || this.inputfrom.trim() === '' || this.content === '') {
        ...
        const { data: res } = await this.$http.post('/souqi/admin/news_manage/change/', {
          admin_pk: updatepk,
          pk: this.msg,
          title: this.inputtitle,
          news_source: this.inputfrom,
          news_content: this.content,
          news_picture: this.imagelast
        })
        // console.log(res)
    ...
    async getdeitartic() {
      ...
      const { data: res } = await this.$http.post('souqi/admin/news_manage/edit/', { pk: this.msg })
      // console.log(res)
      if (res.status !== 1) return
      // 进行赋值渲染到页面
      this.content = res.results.news_content
      this.inputtitle = res.results.title
      this.inputfrom = res.results.news_source
    },
    // 点击清除重置按钮触发事件
    deletall() {
    ...
    async pushartcile() {
      if (this.inputtitle.trim() === '' || this.inputfrom.trim() === '' || this.content === '') {
        ...
        const { data: res } = await this.$http.post('/souqi/admin/news_manage/add/', {
          pk: pkk,
          title: this.inputtitle,
          news_source: this.inputfrom,
    ...
    onEditorBlur(editor) {
      // console.log(editor)
      // console.log(this.content)
      // window.sessionStorage.setItem('zhizhzi', this.content)
    },
    // 富文本编辑器 获得焦点事件
    onEditorFocus(editor) {
      // this.content = window.sessionStorage.getItem('zhizhzi')
      // // console.log(editor)
    },
    // 富文本编辑器内容发生改变的时候
    onChange() {
