<template>
  <div>
    ...
    <el-breadcrumb separator="/">
      ...
      <el-breadcrumb-item>所属资讯列表</el-breadcrumb-item>
    </el-breadcrumb>
    <!-- 卡片视图区域 -->
    <el-card
      v-loading="loading"
    ...
    >
      <el-row :gutter="20">
        ...
        <el-col :span="4">
          <el-button type="primary" class="buttshi" @click="goAddtoolist">添加资讯</el-button>
        </el-col>
      </el-row>
...
export default {
  ...
  created() {
    // 获取页面传递过来的参数
    this.toolvalzhi = this.$route.query.codeid
  },
  mounted() {
    this.duiyingetinfolist()
  },
  watch: {},
  methods: {
    subedit(idzhival) {
    ...
    // 页面加载获取数据总数
    async duiyingetinfolist() {
      this.openloding()
      const { data: res } = await this.$http.post('souqi/admin/tool/consulting/', { tools_name_id: this.toolvalzhi })
      if (res.status !== 1) return this.$message.warning(res.msg)
      if (undefined === res.results) return this.$message.warning('暂无数据')
      ...
      // 将页码赋值
      if (window.sessionStorage.getItem('subpage')) {
        this.queryInfo.pagenum = parseInt(window.sessionStorage.getItem('subpage'))
    ...
    handleCurrentChange(newPage) {
      ...
      window.sessionStorage.setItem('subpage', newPage)
